...
library(ggplot2)
ggplot(df, aes(x=time, y=response)) + geom_line(alpha=0.5) + guides(colour=FALSE) + 
...
colpal <- c("#E69F00", "#56B4E9", "#009E73", "#CC79A7")
ggplot(df, aes(x=time, y=response, color=Version), palette=colpal) + geom_point() + 
xlab("Observation Time Point") + ylab("Observed Response") + 
geom_smooth(se=FALSE, size=1, aes(group=Version)) +
theme(legend.position='top') 
ggsave('Observed_mean_curve.pdf')
...
# ggplot(df, aes(time, Fits_sparse)) + aes(col=Description) +
# geom_point() + geom_smooth(lwd=2, se=FALSE) + 
# ggtitle('Smoothed Response') + xlab('Time') + ylab('pH') + theme_gray()
# ggsave('Smoothed_points_groups.pdf')

# ggplot(df, aes(time, Fits_sparse)) + aes(col=Description) +
# geom_line(alpha=0.2, size=1) + geom_smooth(lwd=2, se=FALSE) + 
# ggtitle('Smoothed Response') + xlab('Time') + ylab('pH') + theme_gray()
# ggsave('Smoothed_lines_groups.pdf')

########## FPC curves ###### (need to automatic ACT different number of PCs)
pdf('FPCs_transformed.pdf', width = 4, height = 4)
plot(time_cont*22, FPC_mean[, 1]*sigma_y + mu_y, type="l", lwd=2, ylim=c(84, 121),
...
pdf('fpcScores.pdf')
par(mfrow=c(1, 2))
boxplot(fpc1 ~ Version, data=df, main='response (PC1)', ylab='FPC1 Score')
boxplot(fpc2 ~ Version, data=df, main='response (PC2)', ylab='FPC2 Score')
dev.off()
...
fit.1 = lm(df$fpc1 ~ factor(df$Version))
summary(fit.1)
#                            Estimate Std. Error t value Pr(>|t|)    
# (Intercept)                -0.28737    0.03727  -7.710 7.41e-14 ***
# factor(df$Version)shift_1x  0.17026    0.05271   3.230  0.00132 ** 
# factor(df$Version)shift_2x  0.34536    0.05271   6.552 1.48e-10 ***
# factor(df$Version)shift_4x  0.69017    0.05271  13.094  < 2e-16 ***  

fit.2 = lm(df$fpc2 ~ factor(df$Version))
summary(fit.2)
#                             Estimate Std. Error t value Pr(>|t|)
# (Intercept)                -0.004746   0.025392  -0.187    0.852
# factor(df$Version)shift_1x  0.002289   0.035910   0.064    0.949
# factor(df$Version)shift_2x  0.007014   0.035910   0.195    0.845
# factor(df$Version)shift_4x  0.011365   0.035910   0.316    0.752
